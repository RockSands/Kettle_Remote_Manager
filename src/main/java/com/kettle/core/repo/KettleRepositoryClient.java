...
public class KettleRepositoryClient {
	 ...
	 * @throws KettleException
	 */
	public synchronized void removeDirectory(String path) throws KettleException {
		connect();
		RepositoryDirectoryInterface rdi = repository.findDirectory(path);
		if (rdi != null) {
			repository.deleteRepositoryDirectory(rdi);
		}
	}
	...
	public synchronized KettleJobEntireDefine getJobEntireDefine(KettleRecord record) throws KettleException {
		...
		if (jobMeta == null) {
			throw new KettleException("Kettle资源库未找到Recode配置[" + record.getUuid() + "]");
		}
	...
	public synchronized void moveJobEntireDefine(KettleRecord record, String newPath) throws KettleException {
		connect();
		ObjectId newID = repository.renameRepositoryDirectory(toObjectID(record.getJobid()), getDirectory(newPath),
				null);
		record.setJobid(newID.getId());
		for (KettleRecordRelation relation : record.getRelations()) {
			newID = repository.renameRepositoryDirectory(toObjectID(relation.getMetaid()), getDirectory(newPath), null);
